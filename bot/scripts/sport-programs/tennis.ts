 ...
 * День 3 - Кондиция+Кор (ОФП) с финишем бегом (игра на корте - вне шаблона,
 * здесь - зальная ОФП + легкий финиш-бег). Взрывная работа -
...
function cardioRun(phase: Phase): ParsedExercise {
  ...
  if (phase === "power") {
    return {
      ...
      pace: "темп 5К",
      heart_rate: "Z4-Z5",
      notes: "интервалы после силовой - восстановление между повторами - легкий бег/ходьба",
    };
  }
  return {
    block: "Кондиция",
...
function buildDayA(w: number, phase: Phase): ParsedExercise[] {
  ...
  ex.push(strength(phase, "Сила", "Подъем на носки (икры)", "полная амплитуда - можно с весом"));
...
function buildDayB(w: number, phase: Phase): ParsedExercise[] {
  ...
  ex.push(strength(phase, "Манжета", "Наружная ротация плеча (кабель, стоя)", "ротаторы - умеренно (на фоне игры)"));
  ex.push(core(phase, "Планка", "40-60с", "анти-экстензия - жесткость туловища"));
...
function buildDayC(w: number, phase: Phase): ParsedExercise[] {
  ...
  if (e1) ex.push(e1);
  ex.push(cond(phase, "Трастеры", "12-15", "с гантелями 2x8-16кг - глубокий присед + жим без паузы"));
  ...
  ex.push(cond(phase, "Берпи", "8-10", "кондиция всего тела"));
...
export function buildTennisProgram(): ParsedContent {
  ...
  for (let w = 1; w <= 12; w++) {
    ...
    const days: ParsedDay[] = [
      ...
      { day_name: "День 3 - Кондиция + Кор (ОФП)", day_order: 3, focus: "Трастеры/махи/фермерская/слэм/берпи + кор + финиш-бег", exercises: buildDayC(w, phase) },
    ];
  ...
  return {
    ...
    notes: [
      ...
      "День 3 - зальная ОФП (кондиция+кор) - финиш-бег легкий, игра на корте - вне шаблона.",
      "Взрывная/плиометрика - строго в начале дня, только фаза power (нед 9-11) - на разгрузке без нее.",
      "Прекауции: плиометрика только после разминки - при боли в плече снизить объем наружной ротации.",
    ],
  };
}
