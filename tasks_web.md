...
| 1.2 | TypeScript конфиг | Создать `bot/tsconfig.json` (target: ES2022, module: NodeNext, strict) | pending |
| 1.3 | Конфигурация | Создать `bot/src/config.ts` — чтение env vars, экспорт объекта конфигурации | pending |
| 1.4 | Supabase клиент | Скопировать `supabase-admin.ts` из `web/src/lib/` в `bot/src/lib/` | pending |
| 1.5 | Типы БД | Скопировать `types/supabase.ts` из `web/src/types/` в `bot/src/lib/types.ts` | pending |
...
| # | Задача | Описание | Статус |
|---|--------|----------|--------|
| 2.1 | Базовый `bot.ts` | Экземпляр `Bot`, middleware для логирования, обработка ошибок | pending |
| 2.2 | Поиск клиента | Функция `findClientByTelegramId(telegramId)` — запрос в Supabase `clients` | pending |
| 2.3 | Команда `/start` | Приветствие: активные клиенты → меню; неактивные → "подключиться по коду"; новые → "купить программу" | pending |
| 2.4 | Подключение по коду | Ввод 8-символьного кода → связка `telegram_id` с клиентом в Supabase | pending |
| 2.5 | Команда `/menu` | Адаптивный список команд на основе `status` и `payment_status` клиента | pending |
| 2.6 | Команда `/myprogram` | Показ текущей программы, ссылки на spreadsheet (если есть), кода подключения | pending |
| 2.7 | Машин состояний | Модуль `state/machine.ts`: `getState`, `setState`, `clearState` (Supabase `bot_state`) | pending |
| 2.8 | Роутинг callback | Обработка inline-кнопок: `today_open`, `exercise_log:*`, `exercise_skip:*`, `skip_workout`, и т.д. | pending |
| 2.9 | Локализация | Утилита `i18n.ts` — выбор языка на основе `client.language` | pending |
...
| # | Задача | Описание | Статус |
|---|--------|----------|--------|
| 3.1 | Определение тренировки | Функция `getTodayWorkout(client)` — определение дня недели → поиск в `program_schedule` → чтение плана | pending |
| 3.2 | Чтение плана тренировки | Функция `getWorkoutPlan(client, scheduleWeek)` — чтение из Supabase или legacy spreadsheet | pending |
| 3.3 | Показ упражнения | Кнопки: "Выполнил" (логирование), "Пропустить", навигация по упражнениям | pending |
| 3.4 | Wizard логирования | 5 шагов: подходы → повторы → вес → RPE → комментарий | pending |
...
| # | Задача | Описание | Статус |
|---|--------|----------|--------|
| 4.1 | Начало замеров | Команда или кнопка → запуск 14-шагового wizard | pending |
| 4.2 | Wizard замеров | 14 шагов: вес → талия → живот → грудь → бёдра → ягодицы → левое бедро → правое бедро → левая рука → правая рука → % жира → мышечная масса → висцеральный жир → комментарий | pending |
| 4.3 | Запись замеров | INSERT в `measurements` (Supabase) | pending |
| 4.4 | Показ динамики | Последние 4 замера: дельта веса, талии, живота, груди, бёдер, % жира | pending |
| 4.5 | Загрузка фото | Приём фото от пользователя → скачивание через Telegram API → загрузка в Supabase Storage | pending |
| 4.6 | Метаданные фото | INSERT в `photos` (Supabase): client_id, week, type (front/side/back/composition), storage_path | pending |
| 4.7 | Прогресс-фото | Просмотр последних фото в боте (отправка обратно в чат) | pending |
...
| # | Задача | Описание | Статус |
|---|--------|----------|--------|
| 5.1 | Начало чек-ина | `/checkin` → запуск 7-шагового опроса | pending |
| 5.2 | Wizard чек-ина | 7 шагов: самочувствие(1-10) → сон(часы) → стресс(1-10) → adherence(0-100%) → пропущенные(кол-во) → жалобы(текст) → комментарий | pending |
| 5.3 | Запись чек-ина | INSERT в `checkins` (Supabase) | pending |
| 5.4 | Уведомление коучу | Отправка сводки чек-ина коучу в Telegram | pending |
