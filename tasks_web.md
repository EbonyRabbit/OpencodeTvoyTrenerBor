...
| # | Задача | Описание | Статус |
|---|--------|----------|--------|
| **9.1** | Миграция БД: purchase поля | Добавить в `clients`: `purchase_date TIMESTAMPTZ`, `purchased_program_id UUID REFERENCES programs(id)` | pending |
| **9.2** | Обновить типы | В `web/src/types/supabase.ts` добавить `purchase_date`, `purchased_program_id` в тип `clients.Row` | pending |
...
| **9.7** | Fix фото из Supabase Storage | В `web/src/app/clients/[id]/_components/client-profile.tsx` и `photo-gallery.tsx`: добавить отображение фото через `storage_path` (Supabase Storage URL) если `drive_url = null` | pending |
| **9.8** | История замеров в боте | Новая команда `/my Measurements` или кнопка "История замеров" — показ последних 10 замеров с дельтами | pending |
| **9.9** | История фото в боте | Отправка последних фото (front/side/back) клиенту в чат по запросу | pending |
| **9.10** | История тренировок в боте | Новая команда `/mystats` — количество выполненных тренировок, пропусков, средний RPE за месяц | pending |
| **9.11** | UI создания клиента | Добавить форму "Добавить клиента" на `/clients` — имя, telegram_id (опционально), язык, timezone, статус оплаты | pending |
