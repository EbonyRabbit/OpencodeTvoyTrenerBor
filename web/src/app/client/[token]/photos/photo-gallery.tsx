...
export function PhotoGallery({ photos }: { photos: PhotoRow[] }) {
  ...
  if (photos.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">История фото</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Загрузите первое фото, чтобы начать отслеживать прогресс
          </p>
        </CardContent>
      </Card>
    );
          ...
          >
            {lightboxPhoto.resolvedUrl && !failedImages.has(lightboxPhoto.id) ? (
              <Image
                ...
                priority
                onError={() =>
                  setFailedImages((prev) => new Set(prev).add(lightboxPhoto.id))
                }
              />
            ) : (
