...
| **12.20** | Фиксы после продакшен-теста | Роутинг `message:text` (state-флоу → игнор неизвестных `/команд` → coach → free-text), команда `/settings` → портал, DB-дедупликация заявок через `bot_dedup` (мульти-инстанс), мультивыбор тренировочных дней (7 дней, ✅/⚪️, plural ru). Commit `a33a384`, ревью 9.5/10 | ✅ (код) |
