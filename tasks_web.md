...
| **12.13** | Тест бота в продакшене | Автоматическая часть выполнена (06.08.2026): `/health` → `{"ok":true}`, webhook `https://tvoi-trener-bot.onrender.com/webhook` активен (pending=0), `client_tokens` генерируются/используются. ⏳ Вручную в Telegram: `/start`, `/menu`, `/myweb` (должен выдать ссылку на портал) | ⏳ (ручной тест команд в Telegram) |
| **12.14** | Тест веба в продакшене | Автоматическая часть выполнена (06.08.2026): `/` → 200, `/login` → 200, `/clients` и `/programs` → 307 (редирект на auth), портал `/client/[token]` → 200. ⏳ Вручную: залогиниться в админку и открыть профиль клиента | ⏳ (ручной логин в админку) |
| **12.15** | Тест end-to-end | Полный цикл: бот → веб → замеры → чек-ин (вручную) | ⏳ |
| **12.16** | Ротировать GitHub PAT | ✅ Полностью: новый fine-grained PAT создан (Contents: read/write), сохранён в osxkeychain, push работает без PAT в URL; remote без токена (`git remote set-url`); 3 коммита запушены (`b046365` — tip origin/main, 06.08.2026); старый PAT `ghp_jAonn…` отозван — API даёт 401; новые коммиты пушатся без ввода кредов | ✅ |
